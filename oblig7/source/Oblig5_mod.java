...
class Oblig5_mod {
	...
	boolean detaljert = false;
        ...
        if (args.length > 0) {
            filnavn = args[0];
	    // Legger til muligheten for å få detaljert utskrift.
	    if (args.length > 1) {
		if (args[1].equals("detaljert")) {
		    detaljert = true;
		}
		else {
		    System.out.println("Bruk argumentet 'detaljert' " +
				       "for detaljert utskrift");
		    return;
		}
	    }
	    // Modifisering ferdig
        ...
        while (!ord[0].equals("a")) {
		    ...
		    // Lagt til info om losninger som antall
		    // løsninger og korteste vei
		    l.skrivUtLosningInfo();
                } else {
